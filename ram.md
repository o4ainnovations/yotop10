...
- **[UI] Mobile-responsive remaining admin pages** — Rewrote 8 admin pages with Tailwind-only mobile-first design:
  1. `admin/alerts/page.tsx` — Complete Tailwind rewrite (was all inline styles). Thresholds: mobile card stack + desktop table (hidden lg:block). Each card: metric name, operator/threshold, severity badge, live status dot, cooldown, enabled toggle, last triggered, delete action. Notifications: glass card list. History: mobile cards + desktop table. Tab bar wraps on mobile. Glass cards, min-h-[44px] touch targets.
  2. `admin/alerts/[id]/page.tsx` — Full-width mobile sections. Sticky bottom resolve button (sticky bottom-0 bg-zinc-950/95 backdrop-blur-sm). Glass card sections.
  3. `admin/search/page.tsx` — Status cards stack vertically (grid-cols-1 sm:grid-cols-3). Reindex/mappings sections full-width. Analytics tabs wrap on mobile. Index table horizontal scroll on mobile. Glass cards.
  4. `admin/audit/page.tsx` — Complete Tailwind rewrite (was all inline styles). Log table converted to mobile card stack + desktop table. Each card: action badge, admin username, IP, date. Filters wrap on mobile. Glass cards.
  5. `admin/hall-of-fame/page.tsx` — Refined mobile cards with glass styling. Reorder ArrowUp/ArrowDown per card. Candidates single-column on mobile (lg:hidden cards + hidden lg grid). Featured/Candidates toggle preserved.
  6. `admin/settings/mods/page.tsx` — ModalShell full-screen on mobile (w-full h-full sm:h-auto sm:max-w-lg sm:rounded-xl). Delete confirm dialog full-screen on mobile. All modals min-h-[44px] touch targets. Glass cards for mod list.
  7. `admin/notifications/page.tsx` — Complete Tailwind rewrite (was all inline styles). Tabs wrap on mobile. Sent messages as glass card stack. Compose form: full-width inputs. Templates as glass card list.
  8. `admin/page.tsx` — Dashboard action cards: 1-col mobile, 2-col sm, 4-col xl with glass styling. Quick links: 2-col grid on mobile with min-h-[44px] touch targets.
  All 8 files: Tailwind CSS only, zero inline styles, zero emoji icons, Lucide Icon component throughout, all existing functionality preserved. tsc --noEmit: 0 errors. ESLint: 0 errors, 0 warnings.
