...
export default function AdminSearchPage() {
  ...
  const primaryBtnClass = (disabled: boolean) => `px-5 py-2 text-white border-none rounded-xl cursor-pointer text-[13px] font-bold min-h-[36px] ${disabled ? 'bg-white/10 cursor-not-allowed' : 'bg-gradient-to-r from-orange-500 to-pink-500 cursor-pointer hover:from-orange-600 hover:to-pink-600'}`;
  const selClass = 'px-2.5 py-1.5 border border-white/10 rounded-lg text-[13px] bg-white/5 text-white outline-none min-h-[36px]';
  const sectionClass = 'bg-white/[0.02] border border-white/5 rounded-2xl p-4 mb-4 w-full';
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex gap-0 mb-5 border-b border-white/10 flex-wrap">
        <button onClick={() => setTab('status')} className={`px-4 py-2.5 border-none bg-transparent cursor-pointer text-sm min-h-[44px] transition-colors ${tab === 'status' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent hover:text-white/60'}`}>Cluster Status</button>
        <button onClick={() => { setTab('analytics'); fetchAnalytics(); }} className={`px-4 py-2.5 border-none bg-transparent cursor-pointer text-sm min-h-[44px] transition-colors ${tab === 'analytics' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent hover:text-white/60'}`}>Search Analytics</button>
      </div>
        ...
        <>
          <div className="mb-5 px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl">
            <div className="flex gap-5 items-center flex-wrap">
              ...
              <button onClick={fetchStatus} className="px-3.5 py-1.5 border border-white/10 rounded-lg bg-white/5 cursor-pointer text-xs text-white min-h-[36px] flex items-center gap-1.5">
          ...
          <div className={sectionClass}>
            ...
            <div className="flex gap-2.5 flex-wrap">
              <input value={previewQ} onChange={e => setPreviewQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && handlePreview()} placeholder="Enter query..." className="flex-1 min-w-[200px] px-3 py-2 border border-white/10 rounded-lg text-[13px] bg-white/5 text-white outline-none placeholder:text-white/30 min-h-[36px]" />
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
              ...
              {trending.length === 0 ? <div className="p-6 text-center text-white/40 text-[13px]">No data yet.</div> : (
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
              ...
              {popular.length === 0 ? <div className="p-6 text-center text-white/40 text-[13px]">No data yet.</div> : (
                <table className="w-full border-collapse text-xs">
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
              ...
              {engaged.length === 0 ? <div className="p-6 text-center text-white/40 text-[13px]">No data yet.</div> : (
                <table className="w-full border-collapse text-xs">
