...
export default function AdminSearchPage() {
  ...
  const primaryBtnClass = (disabled: boolean) => `px-5 py-2 text-white border-none rounded-xl cursor-pointer text-[13px] font-bold min-h-[36px] ${disabled ? 'bg-white/10 cursor-not-allowed' : 'bg-gradient-to-r from-orange-500 to-pink-500 cursor-pointer'}`;
  const selClass = 'px-2.5 py-1.5 border border-white/10 rounded-lg text-[13px] bg-white/5 text-white outline-none min-h-[36px]';
  const sectionClass = 'bg-white/5 border border-white/10 rounded-xl p-4 mb-4';
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex gap-0 mb-5 border-b border-white/10">
        <button onClick={() => setTab('status')} className={`px-4 py-2 border-none bg-transparent cursor-pointer text-sm ${tab === 'status' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent'}`}>Cluster Status</button>
        <button onClick={() => { setTab('analytics'); fetchAnalytics(); }} className={`px-4 py-2 border-none bg-transparent cursor-pointer text-sm ${tab === 'analytics' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent'}`}>Search Analytics</button>
      </div>
        ...
        <>
          <div className="mb-5 px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl">
            <div className="flex gap-5 items-center">
              ...
              <button onClick={fetchStatus} className="px-3.5 py-1.5 border border-white/10 rounded-lg bg-white/5 cursor-pointer text-xs text-white min-h-[36px]">
          ...
          <div className={sectionClass}>
            <h3 className="text-[15px] font-bold mb-3 text-white flex items-center gap-2"><Icon name="RefreshCw" size={16} /> Reindex</h3>
          ...
          <div className={sectionClass}>
            <h3 className="text-[15px] font-bold mb-3 text-white flex items-center gap-2"><Icon name="Search" size={16} /> Test Query</h3>
            <div className="flex gap-2.5">
              <input value={previewQ} onChange={e => setPreviewQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && handlePreview()} placeholder="Enter query..." className="flex-1 px-3 py-2 border border-white/10 rounded-lg text-[13px] bg-white/5 text-white outline-none placeholder:text-white/30" />
              <button onClick={handlePreview} className={primaryBtnClass(false)}>Search</button>
            </div>
            {previewResult && <div className="mt-3 text-[13px]">
          ...
          <div className={sectionClass}>
            <h3 className="text-[15px] font-bold mb-3 text-white flex items-center gap-2"><Icon name="ClipboardList" size={16} /> Mappings</h3>
            <button onClick={handleShowMappings} className={primaryBtnClass(false)}>View Mappings</button>
            {showMappings && mappings && <pre className="mt-3 text-[11px] bg-white/[0.03] p-3 rounded-lg max-h-[300px] overflow-auto text-white/60 border border-white/10">{JSON.stringify(mappings, null, 2)}</pre>}
          </div>

          <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-4 mb-4">
            <h3 className="text-[15px] font-bold mb-3 text-white flex items-center gap-2"><Icon name="TriangleAlert" size={16} color="#e65100" /> Delete Index</h3>
            <div className="flex gap-2.5 items-center">
              ...
              <button onClick={handleDeleteIndex} disabled={!deleteIdx} className={`px-5 py-2 text-white border-none rounded-xl text-[13px] font-bold min-h-[36px] ${deleteIdx ? 'bg-red-700 cursor-pointer hover:bg-red-600' : 'bg-white/10 cursor-not-allowed'}`}>Delete & Recreate</button>
            </div>
          </div>
        </>
      )}
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
              ...
              <div className="text-xs text-white/60 mt-1">Trending Now</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-orange-400">{popular.length}</div>
              <div className="text-xs text-white/60 mt-1">Popular (7d)</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-green-500">{engaged.length}</div>
              <div className="text-xs text-white/60 mt-1">Most Engaged</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-[11px] text-white/40 mb-2.5">EMA spike detection: current hour vs 24h baseline</p>
              {trending.length === 0 ? <div className="p-6 text-center text-white/40 text-[13px]">No data yet — searches will appear here as users search</div> : (
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-[11px] text-white/40 mb-2.5">Decay-weighted rank over 7 days (0.95^d)</p>
              {popular.length === 0 ? <div className="p-6 text-center text-white/40 text-[13px]">No data yet — searches will appear here as users search</div> : (
                <table className="w-full border-collapse text-xs">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-[11px] text-white/40 mb-2.5">Bayesian CTR (alpha=1, beta=99 prior)</p>
              {engaged.length === 0 ? <div className="p-6 text-center text-white/40 text-[13px]">No data yet — searches will appear here as users search</div> : (
                <table className="w-full border-collapse text-xs">
