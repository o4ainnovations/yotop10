...
export default function AdminCategoriesPage() {
  ...
  const primaryBtnClass = (disabled?: boolean) => `px-4 py-2 text-white border-none rounded-xl cursor-pointer text-xs font-bold min-h-[44px] ${disabled ? 'bg-white/10 cursor-not-allowed' : 'bg-gradient-to-r from-orange-500 to-pink-500 cursor-pointer'}`;
  const inputClass = 'px-2.5 py-2 border border-white/10 rounded-lg text-xs bg-white/5 text-white outline-none w-full sm:w-auto min-h-[44px]';
  const cardClass = 'bg-white/[0.02] border border-white/5 rounded-2xl p-4 mb-4';
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      {showCreate && (
        <div className="mb-4 p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
          <div className="flex-1"><label className="text-[11px] text-white/40 block mb-1">Name *</label><input value={newName} onChange={e => setNewName(e.target.value)} className={inputClass} /></div>
          <div className="flex-1"><label className="text-[11px] text-white/40 block mb-1">Slug</label><input value={newSlug} onChange={e => setNewSlug(e.target.value)} className={inputClass} /></div>
          <div className="flex-1"><label className="text-[11px] text-white/40 block mb-1">Parent</label><select value={newParent} onChange={e => setNewParent(e.target.value)} className={inputClass}><option value="" className="bg-zinc-900">None (parent)</option>{parentOptions.map(p => <option key={p.id} value={p.id} className="bg-zinc-900">{p.name}</option>)}</select></div>
      ...
      <div className="flex mb-4 border-b border-white/10 overflow-x-auto -mx-2 px-2">
        <button onClick={() => setTab('tree')} className={`px-4 py-2.5 border-none bg-transparent cursor-pointer text-[13px] whitespace-nowrap min-h-[44px] ${tab === 'tree' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent'}`}>Tree</button>
        <button onClick={() => setTab('table')} className={`px-4 py-2.5 border-none bg-transparent cursor-pointer text-[13px] whitespace-nowrap min-h-[44px] ${tab === 'table' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent'}`}>Table</button>
        <button onClick={() => setTab('analytics')} className={`px-4 py-2.5 border-none bg-transparent cursor-pointer text-[13px] whitespace-nowrap min-h-[44px] ${tab === 'analytics' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent'}`}>Analytics</button>
        <button onClick={() => setTab('bulk')} className={`px-4 py-2.5 border-none bg-transparent cursor-pointer text-[13px] whitespace-nowrap min-h-[44px] ${tab === 'bulk' ? 'font-bold text-orange-400 border-b-2 border-orange-400' : 'text-white/40 border-b-2 border-transparent'}`}>Bulk</button>
      </div>
      ...
      {tab === 'tree' && (
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="w-full sm:w-[320px] flex-shrink-0 max-h-[70vh] overflow-auto border border-white/10 rounded-xl p-3 bg-white/[0.02]">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="w-full px-2.5 py-2.5 border border-white/10 rounded-lg text-xs mb-2.5 bg-white/5 text-white outline-none min-h-[44px]" />
            {parents.map(p => {
              const kids = p.children || [];
              const isOpen = expanded.has(p.id);
              return <div key={p.id}>
                <div onClick={() => { toggleExpanded(p.id); selectCat(p); }}
                  className={`px-2.5 py-2.5 cursor-pointer rounded-lg font-bold text-[13px] flex items-center gap-1.5 text-white min-h-[44px] ${selected?.id === p.id ? 'bg-orange-500/10' : 'bg-transparent'}`}>
                  <span className="text-[10px] text-white/40 w-3 flex-shrink-0">{isOpen ? '\u25BC' : '\u25B6'}</span>
                  <span><Icon name="Folder" size={14} /></span> <span className="flex-1 truncate">{p.name}</span>
                  <span className="text-[11px] text-white/40 font-mono tabular-nums flex-shrink-0">{p.post_count}</span>
                  {p.is_featured && <Icon name="Star" size={10} color="#f57c00" />}
                  {p.status === 'draft' && <span className="text-[10px] bg-orange-500/15 px-1 py-px rounded text-orange-400 flex-shrink-0">draft</span>}
                </div>
                {isOpen && kids.map(c => <div key={c.id} onClick={() => selectCat(c)}
                  className={`pl-7 py-2 px-2 cursor-pointer rounded-lg text-xs flex items-center gap-1 text-white min-h-[44px] ${selected?.id === c.id ? 'bg-orange-500/10' : 'bg-transparent'}`}>
                  <span><Icon name="FileText" size={12} /></span> <span className="flex-1 truncate">{c.name}</span>
                  <span className="text-[10px] text-white/40 font-mono tabular-nums flex-shrink-0">{c.post_count}</span>
                </div>)}
              </div>;
            })}
          </div>
            ...
            {selected ? (
              <div className={cardClass}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[13px]">
                  <div><label className="text-[11px] text-white/40 block mb-1">Name</label><input value={editName} onChange={e => setEditName(e.target.value)} disabled={!selected.parent_id} className={`${inputClass} ${!selected.parent_id ? 'opacity-50' : ''}`} /></div>
                  <div><label className="text-[11px] text-white/40 block mb-1">Slug</label><input value={editSlug} onChange={e => setEditSlug(e.target.value)} disabled={!selected.parent_id} className={`${inputClass} ${!selected.parent_id ? 'opacity-50' : ''}`} /></div>
                  <div><label className="text-[11px] text-white/40 block mb-1">Status</label>
                  ...
                  <div><label className="text-[11px] text-white/40 block mb-1">Sort Order</label><input type="number" value={editSort} onChange={e => setEditSort(parseInt(e.target.value) || 0)} disabled={!selected.parent_id} className={`${inputClass} ${!selected.parent_id ? 'opacity-50' : ''}`} /></div>
                  <div className="flex items-end pb-1"><label className="text-[11px] text-white/40 flex items-center gap-2 min-h-[44px]"><input type="checkbox" checked={editFeatured} onChange={e => setEditFeatured(e.target.checked)} disabled={!selected.parent_id} className="w-4 h-4" /> Featured</label></div>
                </div>
                {!selected.parent_id && (
                ...
                {selected.parent_id && (
                  <div className="mt-4 flex gap-2 flex-wrap">
                    ...
                    <button onClick={() => handleDuplicate(selected.id)} className="px-4 py-2 bg-orange-500 text-white border-none rounded-xl cursor-pointer text-xs font-bold hover:bg-orange-400 min-h-[44px]">Duplicate</button>
                    {selected.status !== 'published' && <button onClick={() => handlePublish(selected.id)} className="px-4 py-2 bg-green-600 text-white border-none rounded-xl cursor-pointer text-xs font-bold hover:bg-green-500 min-h-[44px]">Publish</button>}
                    {selected.status !== 'hidden' && <button onClick={() => handleHide(selected.id)} className="px-4 py-2 bg-white/10 text-white border-none rounded-xl cursor-pointer text-xs font-bold hover:bg-white/20 min-h-[44px]">Hide</button>}
                    <button onClick={() => handleArchive(selected.id)} className="px-4 py-2 bg-red-700 text-white border-none rounded-xl cursor-pointer text-xs font-bold hover:bg-red-600 min-h-[44px]">Archive</button>
                  </div>
                )}
              </div>
            ) : (
      ...
      {tab === 'table' && (
        <div>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search categories..." className="px-2.5 py-2.5 border border-white/10 rounded-lg text-xs mb-3 w-full sm:w-[250px] bg-white/5 text-white outline-none min-h-[44px]" />
          ...
          <div className="sm:hidden flex flex-col gap-2">
            {filtered.map(c => {
              ...
              return (
                <div key={c.id} className="bg-white/[0.02] border border-white/5 rounded-2xl p-3">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0 flex-1">
                      <div className="text-white text-sm font-semibold truncate">{c.name}</div>
                      <div className="text-white/40 text-[11px] font-mono">{c.slug}</div>
                    </div>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {c.is_featured && <Icon name="Star" size={12} color="#f57c00" />}
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold uppercase tracking-wider whitespace-nowrap ${c.status === 'draft' ? 'bg-orange-500/15 text-orange-400' : c.status === 'hidden' ? 'bg-white/10 text-white/40' : 'bg-green-500/15 text-green-400'}`}>{c.status}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { selectCat(c); setTab('tree'); }} className="flex-1 px-3 py-2 bg-gradient-to-r from-orange-500 to-pink-500 text-white border-none rounded-xl cursor-pointer text-xs font-bold min-h-[44px]">Edit</button>
                    <button onClick={() => handleArchive(c.id)} className="flex-1 px-3 py-2 bg-red-700 text-white border-none rounded-xl cursor-pointer text-xs font-bold min-h-[44px]">Archive</button>
                  </div>
                </div>
              );
            })}
            ...
            <div className={cardClass}>
              ...
              {analytics.distribution.map(d => (
                <div key={d.name} className="mb-2 flex items-center gap-2.5">
                  <span className="w-[120px] text-[13px] font-bold text-white flex-shrink-0 truncate">{d.name}</span>
      ...
      {tab === 'bulk' && (
        <div className="flex flex-col gap-4 max-w-[600px]">
          ...
          <div className={cardClass}>
            ...
            {filtered.map(c => <label key={c.id} className="flex items-center gap-2 text-xs py-1.5 text-white cursor-pointer min-h-[44px]"><input type="checkbox" checked={bulkIds.has(c.id)} onChange={() => { const n = new Set(bulkIds); if (n.has(c.id)) n.delete(c.id); else n.add(c.id); setBulkIds(n); }} className="w-4 h-4" /> {c.name} <span className="text-white/40">({c.slug})</span></label>)}
          </div>
