...
interface UserProfile {
  ...
  created_at: string;
  stats: { member_since: string; total_posts: number; total_comments: number; approval_rate: number };
  posts: Array<{
...
export default function UserProfileClient({ initialProfile }: UserProfileClientProps) {
  ...
  const tabButtonClasses = (tab: 'posts' | 'comments' | 'stats') =>
    `px-4 sm:px-6 py-3 text-sm font-medium transition-colors border-b-2 ${
      activeTab === tab ? 'border-orange-400 text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300'
    }`;
  ...
  return (
    <div className="mx-auto min-h-screen max-w-3xl bg-zinc-950 px-3 py-6 text-white sm:px-6 sm:py-10 sm:pb-16">
      <div className="mb-6 rounded-2xl border border-white/5 bg-white/5 p-5 sm:p-8">
        <div className="flex items-start gap-5">
          <div className="shrink-0">
            {profile.profile_image_url ? (
              <Image src={profile.profile_image_url} alt="" width={80} height={80} className="h-20 w-20 rounded-full border-2 border-white/10 object-cover" />
            ) : (
          ...
          <div className="min-w-0 flex-1">
            <h1 className="mb-2 text-2xl font-bold -tracking-[0.02em] text-white sm:text-3xl">{profile.username}</h1>
            <p className="mb-3 flex flex-wrap items-center gap-x-3 gap-y-1">
              ...
              {profile.is_own_profile && authUser && (
                <span className="text-sm text-zinc-400">Trust Score: {authUser.trust_score.toFixed(2)} / 2.0</span>
              )}
            </p>
        ...
        {profile.is_own_profile && (
          <div className="mt-5 border-t border-white/5 pt-4">
            {imageError && (
              <div role="alert" className="mb-3 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-400">{imageError}</div>
            )}
            <label className="inline-flex cursor-pointer items-center gap-2 text-xs text-zinc-500 transition hover:text-zinc-400">
              ...
              {uploadingImage ? (
                <span className="inline-flex items-center gap-1.5 text-zinc-400">
                  <Icon name="RefreshCw" size={12} className="animate-spin" /> Uploading...
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5">
                  <Icon name="Camera" size={12} /> Change profile image
                </span>
              )}
            </label>
          </div>
      ...
      {profile.is_own_profile && (
        <div className="mb-6 rounded-2xl border border-white/5 bg-white/5 p-5 sm:p-8">
          <div className="flex flex-wrap gap-2.5">
            <button onClick={() => setEditingName(true)} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-transparent px-5 py-2.5 text-sm font-medium text-white transition hover:border-orange-500/30 hover:text-orange-400">
              Edit Display Name
            </button>
            <button onClick={() => router.push('/username-history')} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-transparent px-5 py-2.5 text-sm font-medium text-white transition hover:border-orange-500/30 hover:text-orange-400">
              Username History
            </button>
          </div>
          {editingName && (
            <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-4">
              {nameError && <div role="alert" className="mb-2.5 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-400">{nameError}</div>}
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button onClick={handleUpdateDisplayName} className="rounded-xl bg-gradient-to-r from-orange-500 to-pink-500 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-orange-500/25 transition hover:shadow-xl hover:shadow-orange-500/40">Save</button>
                <button onClick={() => setEditingName(false)} className="rounded-xl border border-white/10 bg-transparent px-5 py-2.5 text-sm font-medium text-zinc-400 transition hover:border-orange-500/30 hover:text-orange-400">Cancel</button>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mb-6 flex border-b border-white/5">
        <button onClick={() => setActiveTab('posts')} className={tabButtonClasses('posts')}>Posts ({profile.posts.length})</button>
        <button onClick={() => setActiveTab('comments')} className={tabButtonClasses('comments')}>Comments ({profile.comments.length})</button>
        {profile.is_own_profile && (
          <button onClick={() => setActiveTab('stats')} className={`inline-flex items-center gap-1.5 px-4 sm:px-6 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'stats' ? 'border-orange-400 text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}>
      ...
      {activeTab === 'posts' && (
        <div className="flex flex-col gap-4">
          {profile.posts.length === 0 ? (
            <p className="py-10 text-center text-sm text-zinc-500">No posts yet.</p>
          ) : (
            profile.posts.map((post) => (
              <div key={post.id} className="rounded-xl border border-white/5 bg-white/5 p-5 transition-all duration-300 hover:border-orange-500/30 hover:bg-white/5 sm:p-6">
                <h3 className="mb-2.5 text-base font-semibold leading-snug sm:text-lg">
                  <Link href={`/${post.slug}`} className="text-white transition-colors hover:text-orange-400">{post.title}</Link>
                </h3>
                <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1 text-sm text-zinc-500">
                  <span>{post.category?.name || 'Uncategorized'}</span>
                  <span className="inline-flex items-center gap-1"><Icon name="MessageCircle" size={12} className="text-zinc-500" /> {post.comment_count}</span>
                  <span suppressHydrationWarning>{formatDate(post.created_at)}</span>
                  {profile.is_own_profile && (
                    <span className={`text-xs font-semibold ${post.revision_guidance ? 'text-orange-400' : postStatusStyles[post.status] || 'text-amber-400'}`}>
                      {post.revision_guidance ? 'Revision Requested' : post.status.replace('_', ' ')}
                    </span>
                  )}
                </div>
                {profile.is_own_profile && post.rejection_reason && (
                  <div className="mt-3 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-400"><strong>Reason:</strong> {post.rejection_reason}</div>
                )}
                {profile.is_own_profile && post.revision_guidance && (
                  <div className="mt-3 rounded-lg border border-orange-500/30 bg-orange-500/10 p-3 text-sm text-orange-400"><strong>Admin feedback:</strong> {post.revision_guidance}</div>
                )}
              </div>
            ))
          )}
      ...
      {activeTab === 'comments' && (
        <div className="flex flex-col gap-4">
          {profile.comments.length === 0 ? (
            <p className="py-10 text-center text-sm text-zinc-500">No comments yet.</p>
          ) : (
            profile.comments.map((comment) => (
              <div key={comment.id} className="rounded-xl border border-white/5 bg-white/5 p-5 sm:p-6">
                <p className="mb-2.5 text-sm leading-relaxed text-white sm:text-base">{comment.content}</p>
                <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1 text-sm text-zinc-500">
                  <span className="inline-flex items-center gap-1"><Icon name="Flame" size={12} color="#ea580c" /> {comment.fire_count}</span>
                  <span className="inline-flex items-center gap-1"><Icon name="MessageCircle" size={12} className="text-zinc-500" /> {comment.reply_count}</span>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/5 p-6 sm:p-8">
          <div className="flex flex-col gap-3.5">
            <div className="flex items-center gap-3 rounded-xl bg-white/5 p-4">
              ...
              <span className="text-xl font-bold text-white">{rateLimitStatus.trust_score.toFixed(2)} / 2.0</span>
            </div>
            <div className="flex items-center gap-3 rounded-xl bg-white/5 p-4">
              <span className="text-sm text-zinc-400 sm:text-base">Tier:</span>
              <span className="rounded-lg border border-white/10 bg-white/5 px-2.5 py-0.5 text-sm font-semibold capitalize text-white">{rateLimitStatus.current_tier}</span>
            </div>
            <h3 className="mt-4 flex items-center gap-2 text-sm font-semibold text-white sm:text-base">
              <Icon name="ChartBar" size={16} className="text-zinc-500" /> Current Limits
            </h3>
            <div className="flex flex-col gap-2">
              <p className="flex items-center gap-2 text-sm text-zinc-400"><Icon name="Check" size={14} color="#4ade80" /> Posts: {rateLimitStatus.limits.posts.remaining} / {rateLimitStatus.limits.posts.total} remaining</p>
              <p className="flex items-center gap-2 text-sm text-zinc-400"><Icon name="Check" size={14} color="#4ade80" /> Comments: {rateLimitStatus.limits.comments.remaining} / {rateLimitStatus.limits.comments.total} remaining</p>
              <p className="flex items-center gap-2 text-sm text-zinc-400"><Icon name="Check" size={14} color="#4ade80" /> Counter Lists: {rateLimitStatus.limits.counter_lists.remaining}</p>
            </div>
            <p className="mt-1 flex items-center gap-2 text-sm text-zinc-500">
              <Icon name="RefreshCw" size={14} className="text-zinc-500" /> Resets in: {rateLimitCountdown !== null ? `${Math.floor(rateLimitCountdown / 60)} minutes ${rateLimitCountdown % 60} seconds` : 'Calculating...'}
            </p>
          </div>
        </div>
      )}
